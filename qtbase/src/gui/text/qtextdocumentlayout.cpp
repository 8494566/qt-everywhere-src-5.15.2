...
const auto MarginPixmap = QTextFormat::UserProperty + 1;
const auto MarginPixmapLeft = QTextFormat::UserProperty + 2;
const auto MarginPixmapTop = QTextFormat::UserProperty + 3;
const auto MarginPixmapRight = QTextFormat::UserProperty + 4;
const auto MarginPixmapBottom = QTextFormat::UserProperty + 5;

const auto MarginPixmapLeftOffset = QTextFormat::UserProperty + 6;
const auto MarginPixmapTopOffset = QTextFormat::UserProperty + 7;
const auto MarginPixmapRightOffset = QTextFormat::UserProperty + 8;
const auto MarginPixmapBottomOffset = QTextFormat::UserProperty + 9;
...
void QTextDocumentLayoutPrivate::drawFrameDecoration(QPainter *painter, QTextFrame *frame, QTextFrameData *fd, const QRectF &clip, const QRectF &rect) const
{
    ...
    const QBrush bg = frameFormat.background();
    if (bg != Qt::NoBrush) {
        QRectF bgRect = rect;
    ...
    auto marginPixmapVariant = frameFormat.property(MarginPixmap);
    if (!marginPixmapVariant.isNull())
    {
        auto pixmap = marginPixmapVariant.value<QPixmap>();
        QMargins margins(
            frameFormat.property(MarginPixmapLeft).toInt(),
            frameFormat.property(MarginPixmapTop).toInt(),
            frameFormat.property(MarginPixmapRight).toInt(),
            frameFormat.property(MarginPixmapBottom).toInt()
        );
        auto topMargin = frameFormat.property(MarginPixmapTopOffset).toInt();
        auto leftMargin = frameFormat.property(MarginPixmapLeftOffset).toInt();
        auto bottomMargin = frameFormat.property(MarginPixmapBottomOffset).toInt();
        auto rightMargin = frameFormat.property(MarginPixmapRightOffset).toInt();
        auto pixmapRect = rect.adjusted(leftMargin, topMargin, -rightMargin, -bottomMargin).toRect();
        qDrawBorderPixmap(painter, pixmapRect, margins, pixmap, pixmap.rect(), margins);
    }
                                               ...
                                               QTextBlock *cursorBlockNeedingRepaint, QPointF *cursorBlockOffset) const
{
    ...
    auto tableCellFormat = cell.format().toTableCellFormat();
    auto marginPixmapVariant = tableCellFormat.property(MarginPixmap);
    if (!marginPixmapVariant.isNull())
    {
        auto pixmap = marginPixmapVariant.value<QPixmap>();
        QMargins margins(
            tableCellFormat.property(MarginPixmapLeft).toInt(),
            tableCellFormat.property(MarginPixmapTop).toInt(),
            tableCellFormat.property(MarginPixmapRight).toInt(),
            tableCellFormat.property(MarginPixmapBottom).toInt()
        );
        auto topMargin = tableCellFormat.property(MarginPixmapTopOffset).toInt();
        auto leftMargin =  tableCellFormat.property(MarginPixmapLeftOffset).toInt();
        auto bottomMargin = tableCellFormat.property(MarginPixmapBottomOffset).toInt();
        auto rightMargin = tableCellFormat.property(MarginPixmapRightOffset).toInt();
        auto pixmapRect = cellRect.adjusted(leftMargin, topMargin, -rightMargin, -bottomMargin).toRect();
        qDrawBorderPixmap(painter, pixmapRect, margins, pixmap, pixmap.rect(), margins);
    }
